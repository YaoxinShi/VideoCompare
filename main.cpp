...
int main(int argc, char** argv) {
	try {
        argagg::parser argparser {{
            ...
            { "same", {"-s", "--same"},
            "same position for left and right video (otherwise left and right merge to one full frame)", 0},
        }};
        ...
        if (args["help"] || args.count() == 0) {
            ...
            VideoCompare compare{args["high-dpi"], args["loop"], args["same"], args.pos[0], args.pos[1]};
            compare();
        }
	}
